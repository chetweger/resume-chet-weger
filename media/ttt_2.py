...
def min_search(state, a, b):
  ...
  for child in children:
    #child.print_me()
    next_value = max_search(child, a, b, False)
...
def max_search(state, a, b, is_root):
  #print state.print_me()
  if is_over(state) or is_win(state):
  ...
  for i in range(len(child_util_pairs)):
    next_value = min_search(child_util_pairs[i][0], a, b)
  ...
  if is_root:
    ...
    chosen_child = random.choice(max_states)[0]
...
def ab(state):
  next_move = max_search(state, -9000, 9000, True)
  return next_move
